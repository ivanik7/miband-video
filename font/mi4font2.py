#!/usr/bin/python3
# -*- coding: utf-8 -*-
# Simple python command line tool to pack / unpack the Mi Band 4 font files (v2)
...
def unpackFont(font_path):
	...
	for i in range (0, num_characters):
		...
		img = Image.new('1', (24, 18), 0)
		pixels = img.load()
		char_bytes = font_file.read(54)
		x = 0
		y = 0
		# big endian
		for byte in char_bytes:
			...
			for b in bits:
				...
				if x == 24:
					x = 0
					y += 1
		img.save("bmp" + os.sep + '{:04x}'.format(startrange) + '.bmp') 
...
def packFont(font_path):
	...
	header = bytearray(binascii.unhexlify('4E455A4B02FFFFFFFFFF06000000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000'))
	bmps = bytearray()
	...
	next_part = seq_nr * 54 + range_nr * 6 + 2
